...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.student.awkum.com"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.teacher.awkum.com"
    PORT=9051
  else
    errorln "Org${ORG} unknown (only 1=Student, 2=Teacher supported)"
  fi
...
updateAnchorPeer() {
  set -x
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.awkum.com -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
